...
The Zep node/edge list API uses UUID cursor pagination.
This module encapsulates automatic page traversal logic (with per-page retry),
...
def fetch_all_nodes(
    ...
    """Paginate through graph nodes, returning at most max_items (default 2000). Each page request includes retry."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """Paginate through all graph edges, returning the complete list. Each page request includes retry."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
