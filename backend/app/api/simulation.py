...
logger = get_logger('crucible.api.simulation')


# Interview prompt optimization prefix
# Adding this prefix prevents the Agent from calling tools, responding directly in text
...
def optimize_interview_prompt(prompt: str) -> str:
    """
    Optimize the interview question by adding a prefix to prevent the Agent from calling tools
    ...
    Request (JSON):
        {
            ...
            "graph_id": "crucible_xxxx",    // Optional, fetched from project if not provided
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        # - running: currently running, means preparation was done long ago
        # - completed: run complete, means preparation was done long ago
        # - stopped: stopped, means preparation was done long ago
...
def prepare_simulation():
    ...
    try:
        ...
        # This way the frontend can immediately get the expected Agent total count after calling prepare
        try:
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    Traverses the reports directory to find reports matching simulation_id,
    returns the most recent one if multiple exist (sorted by created_at)
...
def get_simulation_history():
    ...
    Used for homepage history display, returns simulation list with rich info including project name and description
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    Fetch simulation Agent Profiles in real-time (for viewing progress during generation)
...
def generate_profiles():
    """
    Directly generate OASIS Agent Profiles from graph (without creating a simulation)
    
    Request (JSON):
        {
            "graph_id": "crucible_xxxx",     // Required
...
def start_simulation():
    ...
    About the force parameter:
        - When enabled, if the simulation is running or completed, it first stops and clears run logs
    ...
    About enable_graph_memory_update:
        - When enabled, all Agent activities in the simulation (posting, commenting, liking, etc.) are updated to Zep graph in real-time
...
def get_run_status_detail(simulation_id: str):
    ...
    try:
        ...
        # Get current round's actions (recent_actions shows only the latest round)
        ...
        # recent_actions shows only the latest round's content for both platforms
...
def get_simulation_timeline(simulation_id: str):
    ...
    Used for frontend display of progress bar and timeline view
...
def get_agent_stats(simulation_id: str):
    ...
    Used for frontend display of Agent activity rankings, action distributions, etc.
    """
    try:
...
def interview_agent():
    ...
    Note: This feature requires the simulation environment to be in running state (entering command-waiting mode after completing the simulation loop)
    ...
    Returns (without platform, dual-platform mode):
        {
            "success": true,
            "data": {
                "agent_id": 0,
...
def interview_all_agents():
    ...
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",            // Required, simulation ID
            "prompt": "What is your overall view on this matter?",  // Required, interview question (same question for all Agents)
