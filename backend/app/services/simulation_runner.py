...
logger = get_logger('crucible.simulation_runner')

# Flag to track whether the cleanup function has been registered
...
class SimulationRunState:
    ...
    # Recent actions (for real-time display in the frontend)
...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        try:
            ...
            # Append max_rounds to command line args if specified
            if max_rounds is not None and max_rounds > 0:
            ...
            # Set working directory to the simulation directory (databases etc. will be created here).
            ...
            process = subprocess.Popen(
                ...
                start_new_session=True,  # Create new process group; ensures all related processes are terminated when the server shuts down
            )
    ...
    def _monitor_simulation(cls, simulation_id: str):
        ...
        try:
            ...
            # Read logs one final time after process ends
            if os.path.exists(twitter_actions_log):
                    ...
                    if line:
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                # Detect simulation_end event to mark platform as completed
                                if event_type == "simulation_end":
                                    ...
                                    # If both platforms are running, both must be completed.
                                ...
                                # Update round info (from round_end events)
                                    ...
                                    # Overall round is the max across both platforms
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Determines whether a platform is enabled by checking whether its actions.jsonl file exists.
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        ...
        if IS_WINDOWS:
            ...
            try:
                ...
                # Force send SIGKILL if still running after timeout
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Get platform: prefer the record's platform field; fall back to default
    ...
    def get_all_actions(
        ...
        # Fall back to the old single-file format if per-platform files do not exist
        if not actions:
            actions_log = os.path.join(sim_dir, "actions.jsonl")
            actions = cls._read_actions_from_file(
                actions_log,
                default_platform=None,  # Old format should have a platform field
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            )
    ...
    def register_cleanup(cls):
        ...
        # In non-debug mode, this env var is absent, but cleanup still needs to be registered.
        ...
        if is_debug_mode and not is_reloader_process:
            _cleanup_registered = True  # Mark as registered to prevent the child process from retrying
            return
        ...
        try:
            ...
            # Not in the main thread; atexit only
    ...
    def get_env_status_detail(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Get detailed status information about the simulation environment
    ...
    def interview_agent(
        ...
        if not ipc_client.check_env_alive():
            raise ValueError(f"Simulation environment is not running or has closed; cannot perform Interview: {simulation_id}")
    ...
    def interview_all_agents(
        ...
        # Get all agent info from the configuration file
        config_path = os.path.join(sim_dir, "simulation_config.json")
        if not os.path.exists(config_path):
    ...
    def get_interview_history(
        ...
        # Limit total results when querying multiple platforms
