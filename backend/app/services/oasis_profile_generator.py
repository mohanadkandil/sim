...
Optimizations:
1. Calls Zep retrieval functionality to further enrich node information
...
class OasisProfileGenerator:
    ...
    1. Calls Zep graph retrieval functionality to obtain richer context
    ...
    # Group/organization type entities (require generating representative personas)
    GROUP_ENTITY_TYPES = [
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Use Zep graph hybrid search to retrieve rich information related to an entity
        
        Zep does not have a built-in hybrid search interface; edges and nodes must be searched
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        # 1. First attempt to fix truncated content
        content = self._fix_truncated_json(content)
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are an expert in generating social media user personas. Generate detailed, realistic personas for public opinion simulation that maximally reflect existing real-world conditions. Must return valid JSON format; all string values must not contain unescaped newlines."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for the following entity, maximally reflecting existing real-world conditions.
...
2. persona: Detailed persona description (2000 characters of plain text), must include:
   ...
   - Stance and views (attitude toward topics, content likely to provoke or move them)
   - Unique characteristics (catchphrases, special experiences, personal hobbies)
   - Personal memories (important part of persona: describe this individual's connection to events and their existing actions and reactions in those events)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account profile for an organization/group entity, maximally reflecting existing real-world conditions.
...
2. persona: Detailed account profile description (2000 characters of plain text), must include:
   ...
   - Communication style (language characteristics, common expressions, prohibited topics)
   - Content characteristics (content types, posting frequency, active time periods)
   - Stance and attitude (official position on core topics, how controversies are handled)
   - Special notes (profile of the group represented, operational habits)
   - Institutional memory (important part of the persona: describe this organization's connection to events and its existing actions and reactions in those events)
...
- persona must be a coherent block of text; do not use newlines
- age must be the integer 30, gender must be the string "other"
- Institutional account statements must match their identity positioning"""
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        - user_char: Used internally, LLM system prompt, determines how Agent thinks and acts
