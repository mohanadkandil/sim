...
class TimeSimulationConfig:
    ...
    # Time per round (simulation minutes) - default 60 minutes (1 hour), accelerating time flow
    ...
    # Peak period (evening 19-22, most active time for Chinese users)
    peak_hours: List[int] = field(default_factory=lambda: [19, 20, 21, 22])
    peak_activity_multiplier: float = 1.5
    
    # Off-peak period (midnight 0-5, almost no activity)
...
class SimulationConfigGenerator:
    ...
    automatically generating the optimal simulation parameter configuration
...
- User group consists of Chinese users; must follow Beijing time daily schedule habits
- Almost no activity from 0-5am (activity multiplier 0.05)
- Gradually active from 6-8am (activity multiplier 0.4)
- Moderate activity during work hours 9-18 (activity multiplier 0.7)
- Peak period from 19-22 (activity multiplier 1.5)
- Activity declines after 23:00 (activity multiplier 0.5)
- General pattern: low early morning, gradually increasing morning, moderate work hours, evening peak
- **Important**: The example values below are for reference only; adjust specific time periods based on the nature of the event and participant group characteristics
  - For example: student peak may be 21-23; media active all day; official institutions only during work hours
  - For example: a breaking news event may cause discussion even late at night; off_peak_hours can be shortened accordingly
...
**Important**: poster_type must be selected from the "Available Entity Types" above so that initial posts can be assigned to appropriate Agents to publish.
For example: official statements should be published by Official/University type, news by MediaOutlet, student perspectives by Student.
...
{{
    ...
    "reasoning": "<brief explanation>"
        ...
        system_prompt = "You are a public opinion analysis expert. Return pure JSON format. Note that poster_type must precisely match an available entity type."
...
- **Time must conform to Chinese daily schedule**: almost no activity from 0-5am, most active from 19-22 in the evening
